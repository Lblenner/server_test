...
socket.emit("monTag","BLOH")

// GstLaunch found: /bin/gst-launch-1.0
// GStreamer version: 1.16.2
// GStreamer pipeline: v4l2src device=/dev/video0 ! decodebin ! jpegenc ! multipartmux  boundary="--videoboundary" ! tcpserversink host=0.0.0.0 port=10001
...
function GstLiveCamServer(config) {
    ...
    const framerate = config.framerate || config.framerate==0?0:30;
    const deviceIndex = config.deviceIndex || config.deviceIndex==0?0:-1;
    ...
    if (OS.platform() == 'win32')
        ...
        gst_video_src = 'v4l2src device=/dev/video0 ! decodebin';
    else if (OS.platform() == 'darwin')
